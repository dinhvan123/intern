...
{if $_post['post_type'] == "product" && $_post['product']}
    ...
    <div class="mb5 text-bg">
        <i class="fa fa-money-bill-alt fa-fw"></i> <span class="green">{$system['system_currency_symbol']}{$_post['product']['price']|number_format:2:",":"."}</span> ({$system['system_currency']})
    </div>
...
{if $_post['post_type'] == "product" && $_post['product'] && $_post['author_id'] != $user->_data['user_id'] }
    <div class="mt10 clearfix">
        <button type="button" class="btn btn-primary btn-sm pull-right flip js_chat-start" data-uid="{$_post['author_id']}" data-name="{$_post['post_author_name']}"><i class="fa fa-comments"></i> {__("Contact Seller")}</button>
    </div>
{/if}
<!-- product -->